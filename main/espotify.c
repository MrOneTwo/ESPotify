...
// URI setups.
httpd_uri_t uri_get = {
    .uri = "/espotify", .method = HTTP_GET, .handler = get_handler, .user_ctx = NULL};

httpd_uri_t uri_post = {
    .uri = "/espotify", .method = HTTP_POST, .handler = post_handler, .user_ctx = NULL};
...
wifi_init_sta(void)
{
        ...
        .sta =
            {
                ...
                // WEP/W However these modes are deprecated and not advisable to be used. In case
    ...
    // for the maxi number of re-tries (WIFI_FAIL_BIT). The bits are set by event_handler() (see
    // above).
    EventBits_t bits = xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,
                                           pdFALSE, pdFALSE, portMAX_DELAY);

    // xEventGroupWaitBits() returns the bits before the call returned, hence we can test which
    // event actua happened.
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI("espotify", "Connected to ap SSID:%s password:%s", EXAMPLE_ESP_WIFI_SSID,
                 EXAMPLE_ESP_WIFI_PASS);
    } else if (bits & WIFI_FAIL_BIT) {
        ESP_LOGI("espotify", "Failed to connect to SSID:%s, password:%s", EXAMPLE_ESP_WIFI_SSID,
                 EXAMPLE_ESP_WIFI_PASS);
    } else {
...
app_main(void)
{
    ...
    while (1) {
        ...
        // // NOTE(michalc): the ESP_LOGI below flushes the output I think. That's why those prinfs
        // fails
